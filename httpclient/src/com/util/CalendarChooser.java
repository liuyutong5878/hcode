...
/**
 * 
 * @ClassName DateChooser.java
 * @Description 日期选择器，可以指定日期的显示格式
 * 
 * @author 1904
 * @version V1.0
 * @Date 2014年12月24日 下午1:59:12
 */
public class CalendarChooser extends JPanel {
	...
	public static CalendarChooser getInstance() {
		return new CalendarChooser();
	}

	public static CalendarChooser getInstance(Date date) {
		return new CalendarChooser(date);
	}

	public static CalendarChooser getInstance(String format) {
		return new CalendarChooser(format);
	}

	public static CalendarChooser getInstance(Date date, String format) {
		return new CalendarChooser(date, format);
	}
	 ...
	 */
	private CalendarChooser() {
		this(new Date());
	}

	private CalendarChooser(Date date) {
		this(date, "yyyy年MM月dd日");
	}

	private CalendarChooser(String format) {
		this(new Date(), format);
	}

	private CalendarChooser(Date date, String format) {
		initDate = date;
	...
	public void register(final JComponent showDate, final JScrollPane jScrollPane) {
	...
	private class JP1 extends JPanel {
		...
		private void initJP1() {
			...
			this.setPreferredSize(new Dimension(205, 25));
	...
	private class JP2 extends JPanel {
		...
		public JP2() {
			this.setPreferredSize(new Dimension(205, 20));
		}
	...
	private class JP3 extends JPanel {
		...
		public void updateDate() {
			...
			for (int i = 0; i < 42; i++) {
				...
				final MyLabel label = new MyLabel(select.get(Calendar.YEAR), select.get(Calendar.MONTH), select.get(Calendar.DAY_OF_MONTH));
				lm.addLabel(label);
			}
	...
	private class MyLabel extends JLabel implements Comparator<MyLabel>, MouseListener, MouseMotionListener {
		...
		public void setSelected(boolean b, boolean isDrag, boolean isFefresh) {
			isSelected = b;
			if (b && !isDrag) {
		...
		public void mouseEntered(MouseEvent e) {
			...
			Lunar lunar = new Lunar(select, true);
			this.setToolTipText("农历："+lunar.toString());
		}
	...
	private class LabelManager {
		...
		public void setSelect(MyLabel my, boolean b) {
			for (MyLabel m : list) {
				if (m.equals(my)) {
					m.setSelected(true, b, true);
				} else {
					m.setSelected(false, b, true);
				}
			}
		}
	...
	public static void main(String[] args) {
		CalendarChooser dateChooser1 = CalendarChooser.getInstance("yyyy-MM-dd");
		JTextField showDate1 = new JTextField("单击选择日期");
		dateChooser1.register(showDate1, null);
